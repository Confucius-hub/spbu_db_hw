...
_remarks = [
    (
        ...
        "Датасет MKLab Krestenitis (1112 сцен — основа обучения) предоставляется "
        "только в канале VV; включение VH ограничено доступностью исходных данных. "
        "Блоки scSE-внимания частично компенсируют ограничение через "
        "контекстную пространственную фокусировку. "
        "Кросс-поляризационная ветвь запланирована как направление дальнейших исследований.",
        OIL,
    ),
    (
        "Замечание 2: расширение тестовой выборки",
        "Деградация F1: 0,89 (Кольский) → 0,84 (Варандей) → 0,76 (Сабетта) "
        "зафиксирована и подробно проанализирована в §4.4 ВКР. "
        "Сабетта — наиболее сложная акватория (жировой и ниласовый лёд + припай). "
        "Ближайший шаг: целевое дообучение на 10–15 дополнительных сценах Сабетты.",
        BLUE,
    ),
]
_ry = Inches(1.85)
for (_rh, _rb, _racc) in _remarks:
    card(s, Inches(0.7), _ry, Inches(11.95), Inches(2.3),
         bg=CARDBG, border=LIGHTBLUE, accent=_racc)
    txt(s, Inches(1.0), _ry + Inches(0.22), Inches(11.4), Inches(0.55),
        [[(_rh, 16, True, _racc)]], space_after=2)
    txt(s, Inches(1.0), _ry + Inches(0.84), Inches(11.4), Inches(1.3),
        [[(_rb, 13.5, False, DARK)]], line=1.15, space_after=0)
    _ry += Inches(2.6)
